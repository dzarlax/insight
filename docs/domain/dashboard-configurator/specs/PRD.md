...
- **v1.8** (current): Three integration uplifts surfaced by a cross-PRD synthesis with Analytics Diagnosis (PR #298) and the Availability flag work (#364/#365). (1) **Added `diagnosis_verdict_strip` widget type** to the widget enum so Analytics Diagnosis Layer outputs (per-cohort verdicts plus rules → flags → suggested actions) have a top-of-dashboard slot. Without this, Diagnosis has no place to land in a configured dashboard. Updated `cpt-dash-cfg-fr-widget-type-enum` (now 8 initial types) and the §4.1 In-Scope bullet. (2) **Specified empty-state behavior** for known widget types whose underlying metrics all evaluate to "no data to show" — added new FR `cpt-dash-cfg-fr-widget-empty-state` (`p1`). The widget collapses to a ComingSoon placeholder when all bound metrics have `availability = no_source` (per `cpt-metric-cat-fr-availability` in the Metric Catalog) or return NULL across the period; the renderer hides the widget entirely when its `config.hide_when_empty` is true. Closes the gap between metric-level `availability` and widget-level rendering. (3) **Added `default_collapsed: bool` to the standard widget config fields** — new FR `cpt-dash-cfg-fr-widget-default-collapsed` (`p1`). Moves the hardcoded CollapsibleSection `defaultOpen` flag on the frontend into widget configuration, so changing which sections are collapsed by default for a given role is a DB row change instead of a code change. Default is `false` for primary widgets (KPI strip, hero strip, top bullet sections) and `true` for secondary content where the role-default expects users to opt in.
- **v1.7**: Synced with Metric Catalog PRD v1.4+ scope model. The Metric Catalog rejected the `dashboard` scope outright (admins think in role / team / company terms, not dashboard terms; with `role` as a first-class scope, `dashboard` was redundant) and shipped `{ product-default, tenant, role, team, team+role }` as the v1 scope set with precedence `team+role → team → role → tenant → product-default`. Removed `cpt-dash-cfg-fr-dashboard-threshold-scope` (the dashboard scope no longer exists). Updated `cpt-dash-cfg-fr-team-threshold-scope` to reference the new precedence chain and to allow team-lead writes at both `scope = 'team'` and `scope = 'team+role'`. This PRD no longer contributes any threshold scope value of its own; it provides the `role_slug` values (via `role_catalog`) and `team_id` values (via Identity Resolution) that the catalog's scope chain consumes.
...
- Widget-type enum with 8 initial types: `hero_strip`, `bullet_section`, `kpi_strip`, `trend_chart`, `members_table`, `drill_panel`, `coming_soon_banner`, `diagnosis_verdict_strip`
...
The system **MUST** define `widget_type` as an enum covering `hero_strip`, `bullet_section`, `kpi_strip`, `trend_chart`, `members_table`, `drill_panel`, `coming_soon_banner`, and `diagnosis_verdict_strip`. Each `widget_type` **MUST** have a JSON schema validating its `config` on save, rejecting invalid configs before commit. The frontend **MUST** render a `coming_soon_banner` placeholder when it encounters an unknown `widget_type` (e.g., a newer backend than the frontend build) instead of crashing the dashboard. The `diagnosis_verdict_strip` widget **MUST** render the per-cohort verdicts (Green / Yellow / Red / Partial) and flags-with-actions output produced by the Analytics Diagnosis Layer (see PRD #298); its `config` **MUST** identify which cohort scope and rule-set to bind.

**Rationale**: DB stores pure configuration; visual contracts and schemas live in code where they can be type-checked. This matches the Grafana and Superset pattern and keeps schema migrations off the critical path for UI experiments. The `diagnosis_verdict_strip` is the integration point that lets Diagnosis Layer results sit at the top of a configured dashboard so charts become drill-down rather than entry point.

**Actors**: `cpt-dash-cfg-actor-product-team`, `cpt-dash-cfg-actor-analytics-api`

#### Widget Empty-State Behavior

- [ ] `p1` - **ID**: `cpt-dash-cfg-fr-widget-empty-state`

The system **MUST** evaluate each widget's bound metrics against the Metric Catalog `availability` field (see `cpt-metric-cat-fr-availability`) at render time. A widget whose bound metrics all have `availability = no_source` **MUST** be filtered out before render — the widget does not appear, no header, no placeholder. A widget with at least one `live` or `pipeline_pending` binding but where every bound metric returns NULL for the requested period **MUST** render a ComingSoon placeholder in place of its content unless its `config.hide_when_empty` is `true`, in which case it **MUST** be filtered out. The default of `config.hide_when_empty` **MUST** be `false`.

**Rationale**: Without this rule, the configurator surfaces the same "fake zero" / dead-tile problems the honest-NULL pattern (PR #223) and the `availability` field (#364) were introduced to eliminate. The widget layer is the right place to apply the rule because a widget can bind multiple metrics; per-metric hiding alone leaves orphan widget shells.

**Actors**: `cpt-dash-cfg-actor-viewer`, `cpt-dash-cfg-actor-analytics-api`

#### Widget Default-Collapsed Property

- [ ] `p1` - **ID**: `cpt-dash-cfg-fr-widget-default-collapsed`

The standard widget `config` shape **MUST** include an optional `default_collapsed: bool` field (default `false`). When the field is `true` and the widget type supports collapsing, the renderer **MUST** initialize the widget in a collapsed state on first load. Per-user expansion preferences **MAY** be persisted on the client (e.g., localStorage) but **MUST NOT** override the seeded default for users who have not interacted with the widget.

**Rationale**: Section default-collapsed state on the frontend (`CollapsibleSection.defaultOpen`) is currently a hardcoded React prop, which means changing whether AI Tools or Collaboration is open by default for a given role requires a frontend deploy. Moving the flag into the widget config closes that loop and lets product / admins tune the visual density of any role-default dashboard without code.

**Actors**: `cpt-dash-cfg-actor-tenant-admin`, `cpt-dash-cfg-actor-viewer`
